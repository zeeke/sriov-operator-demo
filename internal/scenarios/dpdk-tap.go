...
func makeDpdkSenderContainer() corev1.Container {
	return corev1.Container{
		...
		Args: []string{`
			...
			dpdk-testpmd -a ${PCIDEVICE_OPENSHIFT_IO_DPDKVFIO} --no-huge -m 128M -- \
				--forward-mode=txonly --stats-period=1 --auto-start || sleep inf
		`},
...
func makeDpdkReceiverContainer() corev1.Container {
	return corev1.Container{
		...
		Args: []string{`
			...
			dpdk-testpmd -a ${PCIDEVICE_OPENSHIFT_IO_DPDKVFIO} --no-huge -m 128M --vdev=net_tap0,iface=tap0 -- --forward-mode=io --auto-start --stats-period=1
		`},
		SecurityContext: &corev1.SecurityContext{
			RunAsUser:  ptr.To(int64(0)),
			Privileged: ptr.To(true),
